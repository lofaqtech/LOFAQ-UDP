...
download_hysteria() {
    ...
    local _download_url="$REPO_URL/releases/download/v2.6.2/hysteria-$OPERATING_SYSTEM-$ARCHITECTURE"
...
show_usage_and_exit() {
    echo
    echo -e "\t$(tbold)$SCRIPT_NAME$(treset) - LOFAQ™ UDP Manager install script"
    echo
    echo -e "Usage:"
    echo
    echo -e "$(tbold)Install LOFAQ™ UDP$(treset)"
    ...
    echo -e "\t-l, --local <file>\tInstall specified LOFAQ™ UDP binary instead of downloading it."
    echo -e "\t--version <version>\tInstall specified version instead of the latest."
    echo
    echo -e "$(tbold)Remove LOFAQ™ UDP$(treset)"
    echo -e "\t$0 --remove"
    echo
...
perform_install_manager_script() {
    local _manager_script="/usr/local/bin/lofaq_manager.sh"
    local _symlink_path="/usr/local/bin/lofaq"
    
    echo "Downloading manager script..."
    curl -o "$_manager_script" "https://github.com/lofaqtech/LOFAQ-UDP/raw/main/lofaq_manager.sh"
    chmod +x "$_manager_script"
    
    echo "Creating symbolic link to run the manager script using 'lofaq' command..."
    ln -sf "$_manager_script" "$_symlink_path"
    
    echo "Manager script installed at $_manager_script"
    echo "You can now run the manager using the 'lofaq' command."
}
...
perform_remove() {
    ...
    echo
    echo -e "$(tbold)Congratulations! LOFAQ™ UDP has been successfully removed from your server.$(treset)"
    echo
